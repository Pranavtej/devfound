...
const DevRegister = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.get(`api/Login/dev/${username}/${password}`);
  ...
  const handleCreate = async (event) => {
    ...
    try {
      ...
      const Checkuser = await axios.get(`/dev/find/${newUsername}`);
      ...
      if (Checkuser.status === 404) {
        ...
        document.getElementById("username_err").innerHTML = "Username Already Exist";
            ...
            const response = await axios.post(`/api/CreateAcc/dev/${name}/${newpass}/${email}/${newUsername}`);
            const Newres = await axios.get(`api/Login/dev/${newUsername}/${newpass}`);  
       ...
       <section className="relative h-screen">
        <div className="lg:flex lg:h-full">
          ...
          <div className="relative text-center lg:w-1/2">
            <img src="/assets/img/join_us.png" alt="login" className="absolute h-full w-full object-cover"></img>
        
            <a href="index.html" className="relative inline-block py-36">
              <img src="/assets/img/logo_white.png" className="inline-block max-h-7" alt="Xhibiter | NFT Marketplace"></img>
            </a>
          </div>
          ...
          <div className="relative flex items-center justify-center p-[10%] lg:w-1/2">
            ...
            <div className="w-full max-w-[25.625rem] text-center">
              <h1 className="mb-6 font-display text-4xl text-jacarta-700 dark:text-white">Become  a Dev  !</h1>
